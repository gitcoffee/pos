...
@section('content')
    <div class="content">
        <form method="POST" action="{{ route('admin.pos.restaurants.tables.update', $posTable->id) }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
                    ...
                    <accordian :title="'{{ __('pos::app.admin.restaurants.tables.general') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" id="name" name="name" data-vv-as="&quot;{{ __('pos::app.admin.restaurants.tables.name') }}&quot;" value="{{ $posTable->name }}" />
                                <span class="control-error" v-if="errors.has('name')">@{{ errors.first('name') }}</span>
                            </div>

                            <?php $input_table_type = old('type') ? old('type') : $posTable->type ?>
                            ...
                            <div class="control-group">
                                ...
                                <select class="control" id="status" name="status">
                                    <option value="0" {{ $posTable->status ? '' : 'selected' }}>
                                        {{ __('pos::app.admin.restaurants.tables.inactive') }}
                                    </option>
                                    <option value="1" {{ $posTable->status ? 'selected' : '' }}>
                            ...
                            <div class="control-group" :class="[errors.has('position') ? 'has-error' : '']">
                                ...
                                <input type="number" v-validate="'required|numeric'" class="control" id="position" name="position" data-vv-as="&quot;{{ __('pos::app.admin.restaurants.tables.position') }}&quot;"  value="{{ $posTable->position }}" />
                    ...
                    <accordian :title="'{{ __('pos::app.admin.restaurants.tables.seat-and-agent') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('no_of_seat') ? 'has-error' : '']">
                                ...
                                <input type="number" v-validate="'required|numeric|min_value:1|max_value:20'" class="control" id="no_of_seat" name="no_of_seat" data-vv-as="&quot;{{ __('pos::app.admin.restaurants.tables.total-seat') }}&quot;"  value="{{ $posTable->no_of_seat }}" />
                            ...
                            <div class="control-group" :class="[errors.has('agent_id') ? 'has-error' : '']">
                                ...
                                <select class="control" name="agent_id" v-validate="'required'" id="agent_id" data-vv-as="&quot;{{ __('pos::app.admin.restaurants.tables.assign-agent') }}&quot;">
                                    ...
                                    @foreach ($posUsers as $agent)
                                        <option value="{{ $agent->id }}" {{ ($posTable->agent_id == $agent->id) ? 'selected' : '' }}>{{ $agent->name }}</option>
                                    @endforeach
                                </select>
