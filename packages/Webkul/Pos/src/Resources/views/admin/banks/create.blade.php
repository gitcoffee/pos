...
@section('content')
    <div class="content">
        <form method="POST" action="{{ route('admin.pos.banks.store') }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
                    ...
                    <accordian :title="'{{ __('pos::app.admin.banks.general') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" id="name" name="name" data-vv-as="&quot;{{ __('pos::app.admin.banks.name') }}&quot;" value="{{ old('name') }}" />
                            ...
                            <div class="control-group" :class="[errors.has('address') ? 'has-error' : '']">
                                ...
                                <textarea v-validate="'required'" class="control" id="address" name="address" data-vv-as="&quot;{{ __('pos::app.admin.banks.address') }}&quot;">{{ old('address') }}</textarea>
                            ...
                            <div class="control-group" :class="[errors.has('email') ? 'has-error' : '']">
                                ...
                                <input type="email" v-validate="'email'" class="control" id="email" name="email" data-vv-as="&quot;{{ __('pos::app.admin.banks.email') }}&quot;" value="{{ old('email') }}" />
                                <span class="control-error" v-if="errors.has('email')">@{{ errors.first('email') }}</span>
                            </div>

                            <div class="control-group" :class="[errors.has('phone') ? 'has-error' : '']">
                                <label for="phone">{{ __('pos::app.admin.banks.phone') }}</label>
                                <input type="text" v-validate="'numeric|max:13'" class="control" id="phone" name="phone" data-vv-as="&quot;{{ __('pos::app.admin.banks.phone') }}&quot;" value="{{ old('phone') }}" />
                            ...
                            <div class="control-group" :class="[errors.has('agent_id') ? 'has-error' : '']">
                                ...
                                <select class="control" name="agent_id" v-validate="'required'" id="agent_id" data-vv-as="&quot;{{ __('pos::app.admin.banks.agent') }}&quot;">
                                    ...
                                    @foreach ($agents as $agent)
                                        <option value="{{ $agent->id }}" {{ (isset($agent_id) && $agent_id == $agent->id) ? 'selected' : '' }}>{{ $agent->firstname .' '. $agent->lastname }}</option>
                                    @endforeach
                                </select>
